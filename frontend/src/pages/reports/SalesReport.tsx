...
type SalesDim = 'customer' | 'product' | 'supplier'
...
export default function SalesReport() {
  ...
  const margin = filteredStats.totalAmount
    ? (filteredStats.totalProfit / filteredStats.totalAmount * 100).toFixed(1)
    : null
  ...
  const handleViewL1 = async (row: ReportGroupRow) => {
    ...
    try {
      ...
      if (!l2List) {
        l2List = await getSalesSubgroup({ by: groupBy, parentId: row.id, startDate, endDate })
      ...
      const results = await Promise.allSettled(
        l2List.map(async l2Row => {
          ...
          const params: Parameters<typeof getSalesOrderRows>[0] = { startDate, endDate }
          if (groupBy === 'customer') { params.customerId = row.id; params.productId = l2Row.id }
          else if (groupBy === 'product') { params.productId = row.id; params.customerId = l2Row.id }
          else { params.supplierId = row.id; params.productId = l2Row.id }
  ...
  const l2LabelTitle = groupBy === 'customer' ? '货品' : groupBy === 'product' ? '客户' : '货品'
  const l1LabelTitle = groupBy === 'customer' ? '客户' : groupBy === 'product' ? '货品' : '供应商'
  ...
  return (
    <>
      ...
      <div className={styles.statsRow}>
        ...
        <div className={styles.statCard}>
          <div className={styles.statLabel}>合计毛利</div>
          <div className={filteredStats.totalProfit >= 0 ? styles.statValueGreen : styles.statValueRed}>
