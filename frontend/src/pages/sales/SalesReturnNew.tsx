...
export default function SalesReturnNew() {
    ...
    getSalesReturn(Number(id))
      .then(ret => {
        ...
        if (ret.originalOrderId) {
          getSalesOrder(ret.originalOrderId).then(setSelectedOrder).catch(() => {})
        }
        setLines(
          (ret.items ?? []).map(item => ({
            key: keyCounter++,
  ...
  const handleSearch = (kw: string) => {
    setSearching(true)
    getSalesOrders({ search: kw || undefined })
      .then(setOrders)
      .catch(() => {})
  ...
  const handleSubmit = () => {
    if (!selectedOrder && !isEdit) { message.warning('请先选择原销售单'); return }
    ...
    form.validateFields().then(values => {
      ...
      req
        .then(ret => {
          ...
          navigate(isEdit ? '/sales/returns' : `/sales/returns/${ret.id}`)
        })
        .catch(err => message.error(getErrorMessage(err)))
  ...
  const orderColumns: ColumnsType<SalesOrder> = [
    ...
    { title: '客户', width: 150, render: (_, r) => `${r.customerCode} ${r.customerName}` },
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/sales/returns')}>返回</Button>
        <span className={styles.pageTitle} style={{ margin: 0 }}>{isEdit ? '编辑退货单' : '销售退货'}</span>
      </div>

      {/* Step 1: Select original order */}
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontWeight: 600, marginBottom: 8, color: '#555' }}>选择原销售单</div>
        <Input
          prefix={<SearchOutlined />}
          placeholder="搜索单号、客户名称/编码、货品名称/编码"
          allowClear
          value={orderSearch}
      ...
      {selectedOrder && (
        <div style={{
          ...
          &emsp;销售金额 <strong>¥{selectedOrder.totalAmount.toFixed(2)}</strong>
